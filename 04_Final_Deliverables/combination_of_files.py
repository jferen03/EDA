...
Week 2 Exercise — combines every standalone exploratory data analysis (EDA)
script built for the wildcat loans dataset into a single runnable script,
in pipeline order:

  1.  eda_inspect.py                   — shape, dtypes, missing value counts
  2.  eda_convert_dates.py              — convert origination_date to datetime
  3.  eda_descriptive_stats.py          — descriptive statistics (numeric cols)
...
The CSV is loaded once and shared across all sections below (each original
script loaded its own copy). Chart color constants that were duplicated
across the three plotting scripts (hist, box, scatter) have been
consolidated into one shared palette. Every section's logic and printed
output is otherwise unchanged from its original script.
...
# Colors (light-mode chart palette) - shared across the histogram, box plot,
# and scatter plot sections below, so loan_status colors stay consistent.
SURFACE = "#fcfcfb"
...
def section_01_inspect(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 1. INITIAL INSPECTION  (eda_inspect.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# 2. eda_convert_dates.py
# ---------------------------------------------------------------------------
def section_02_convert_dates(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 2. CONVERT origination_date TO DATETIME  (eda_convert_dates.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# 3. eda_descriptive_stats.py
# ---------------------------------------------------------------------------
def section_03_descriptive_stats(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 3. DESCRIPTIVE STATISTICS  (eda_descriptive_stats.py)")
    print("############################################################")
...
def section_04_missingness_analysis(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 4. MISSINGNESS ANALYSIS  (eda_missingness_analysis.py)")
    print("############################################################")

    # Scoped to this section only, so it matches eda_missingness_analysis.py's
    # own float formatting without changing how other sections display floats.
...
# ---------------------------------------------------------------------------
# 5. eda_categorical_frequencies.py
# ---------------------------------------------------------------------------
...
def section_05_categorical_frequencies(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 5. CATEGORICAL FREQUENCIES  (eda_categorical_frequencies.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# 6. eda_groupby_loan_status.py
# ---------------------------------------------------------------------------
def section_06_groupby_loan_status(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 6. GROUP-BY SUMMARY BY loan_status  (eda_groupby_loan_status.py)")
    print("############################################################")
...
def section_07_correlation_matrix(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 7. CORRELATION MATRIX  (eda_correlation_matrix.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# 8. eda_hist_loan_amount.py
# ---------------------------------------------------------------------------
def section_08_hist_loan_amount(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 8. HISTOGRAM: loan_amount  (eda_hist_loan_amount.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# 9. eda_box_interest_by_status.py
# ---------------------------------------------------------------------------
def section_09_box_interest_by_status(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 9. BOX PLOT: interest_rate BY loan_status  (eda_box_interest_by_status.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# 10. eda_scatter_credit_rate.py
# ---------------------------------------------------------------------------
def section_10_scatter_credit_rate(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 10. SCATTER PLOT: credit_score vs. interest_rate  (eda_scatter_credit_rate.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# 11. eda_default_rate_by_purpose.py
# ---------------------------------------------------------------------------
def section_11_default_rate_by_purpose(df: pd.DataFrame) -> None:
    print("############################################################")
    print("# 11. DEFAULT RATE BY loan_purpose  (eda_default_rate_by_purpose.py)")
    print("############################################################")
...
# ---------------------------------------------------------------------------
# Entry point - runs all sections in pipeline order against one shared load
# of the CSV, and prints everything to the screen.
# ---------------------------------------------------------------------------
def main() -> None:
